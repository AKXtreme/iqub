...
/// Single Iqub by ID
final iqubProvider =
    StreamProvider.family<IqubModel?, String>((ref, iqubId) {
  return ref.watch(iqubRepositoryProvider).watchIqub(iqubId);
});

/// Members of a specific Iqub
final membersProvider =
    StreamProvider.family<List<MemberModel>, String>((ref, iqubId) {
  return ref.watch(iqubRepositoryProvider).watchMembers(iqubId);
});

/// Payments for a specific round  ({iqubId, round} tuple)
final paymentsForRoundProvider = StreamProvider.family<List<PaymentModel>,
    ({String iqubId, int round})>((ref, args) {
  return ref
...
class IqubActionsNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  Future<bool> addMember({
    ...
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() => _repo.addMember(
          iqubId: iqubId,
          name: name,
          phone: phone,
          userId: userId,
  ...
  Future<bool> removeMember(String iqubId, MemberModel member) async {
    ...
    state = await AsyncValue.guard(
        () => _repo.removeMember(iqubId, member));
    return !state.hasError;
  }
  ...
  Future<bool> generatePayments({
    ...
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() => _repo.generatePaymentsForRound(
          iqubId: iqubId,
  ...
  Future<bool> recordPayoutAndAdvance({
    ...
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() => _repo.recordPayoutAndAdvanceRound(
          iqub: iqub,
          recipient: recipient,
        ));
    return !state.hasError;
